#!/usr/bin/env node
/*
  Backfill artist_name and artist_url for hairstyles in local SQLite.
  Requirements:
    - env UNSPLASH_ACCESS_KEY: Unsplash API access key
  Usage:
    node backend/scripts/backfill_unsplash_artists_sqlite.js [--dry]
*/
...
async function main() {
  try {
    const rows = await all(
      `SELECT id, image_url, unsplash_photo_id, artist_name, artist_url FROM hairstyles ORDER BY created_at ASC`
    );
      ...
      const changes = await run(
        `UPDATE hairstyles
         SET artist_name = COALESCE(?, artist_name),
             ...
             unsplash_photo_id = COALESCE(?, unsplash_photo_id),
